const templates = {
  company: {
    label: 'Company',
    title: 'Start a company workspace',
    summary: 'Shared spaces for a team. Review the scopes and channels before creating them.',
    button: 'Create company spaces',
    access: 'Private until people or groups are granted to channels.',
    scopes: [
      {
        name: 'Company HQ',
  ...
  personal: {
    label: 'Personal',
    title: 'Start a personal workspace',
    summary: 'Private spaces for one person. Add people or agents later by granting channels.',
    button: 'Create personal spaces',
    access: 'Owner only until another person or agent is granted to a channel.',
    scopes: [
      {
        name: 'Personal HQ',
...
function render() {
  ...
  accessCopy.textContent = template.access;
  ...
  scope.channels.forEach((channel) => {
    ...
    card.innerHTML = `<strong>${channel}</strong><span>Channel</span>`;
