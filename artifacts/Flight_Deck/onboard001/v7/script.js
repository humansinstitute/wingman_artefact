...
function renderTowerWrites() {
  ...
  scopeChip.textContent = counts.scopes > 0 ? `${counts.scopes} scopes ready` : 'No scopes selected';
  channelChip.textContent = `${counts.channels} channels ready`;
}
...
function renderSummary() {
  ...
  summary.textContent = step === 1
    ...
    : step === 2
      ...
      : step === 5
        ? `${counts.scopes} scopes and ${counts.channels} channels selected.`
        : step === 6
          ? 'Tower remains the source of truth; Flight Deck updates after the PG writes return.'
